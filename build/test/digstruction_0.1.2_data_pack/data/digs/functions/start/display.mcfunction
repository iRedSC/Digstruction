...
execute if score $display.cycle ds.const matches 2 run title @a subtitle {"text":"Watch out for arrows, or you might be in for a surprise","bold":true,"color":"#10c9ea"}
execute if score $display.cycle ds.const matches 3 run title @a subtitle {"text":"The border shrinks after 5 minutes","bold":true,"color":"#ea6710"}
execute if score $display.cycle ds.const matches 4 run title @a subtitle {"text":"Punch tnt to ignite it","bold":true,"color":"#d93636"}
execute if score $display.cycle ds.const matches 5 run title @a subtitle {"text":"Game will start soon...","bold":true,"color":"#ced936"}
execute if score $display.cycle ds.const matches 6 run title @a subtitle {"text":"Lake drainers can be very dangerous","bold":true,"color":"#848cd7"}
execute if score $display.cycle ds.const matches 7 run title @a subtitle {"text":"Slime pads might save you in a rut, but they don't last long","bold":true,"color":"#d5e7e0"}
execute if score $display.cycle ds.const matches 8 run title @a subtitle {"text":"Seek shelter underground from an air strike","bold":true,"color":"#23d18c"}
execute if score $display.cycle ds.const matches 9 run title @a subtitle {"text":"Towers have the best loot, but the hardest enemies","bold":true,"color":"#d12349"}


scoreboard players add $display.cycle ds.const 1
execute if score $display.cycle ds.const matches 10.. run scoreboard players set $display.cycle ds.const 0

particle explosion 0 -10 0 2 0 2 0 50 force @a 

execute if score $game.mode ds.const matches -1 run schedule function digs:start/display 6s replace
